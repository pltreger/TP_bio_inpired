...
class Neural_Network:
    def __init__(self, nb_actions, sizeBuffer, ENVIRONNEMENT):
        #initialisation du model_anticipation
        self.device = torch.device("cpu")
        self.model_anticipation = Net(nb_actions, ENVIRONNEMENT, sizeBuffer).to(self.device)
        self.model_apprentissage = Net(nb_actions, ENVIRONNEMENT, sizeBuffer).to(self.device)

        self.nb_actions = nb_actions

        self.buffer = Buffer(sizeBuffer, ENVIRONNEMENT)
    ...
    def learn(self, sizeBatch, ENVIRONNEMENT, gamma):
        ...
        if self.buffer.index < sizeBatch:
            return

        # On récupère un minibatch
        states, actions, nextstates, rewards, done = self.buffer.get_minibatch(sizeBatch, ENVIRONNEMENT)
        ...
        for weights in dict_apprentissate:
            dict_apprentissate[weights] = (1 - 0.005) * dict_apprentissate[weights] + 0.005 * dict_anticipation[weights]
